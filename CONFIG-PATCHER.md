...
You can automate the update process to run weekly (or any schedule you prefer):

#### 1. Create Update Script

Create `/usr/local/bin/update-neowx.sh`:

```bash
#!/bin/bash
# NeoWX Material automatic update script

# Configuration
SKIN_DIR="/etc/weewx/skins/neowx-material"
LOG_FILE="/var/log/neowx-update.log"
...
log "Starting NeoWX Material update..."

# Update the skin
log "Installing latest version..."
if weectl extension install https://github.com/seehase/neowx-material/archive/refs/heads/master.zip; then
    log "✓ Extension installed successfully"
else
    log "✗ Extension installation failed"
    exit 1
fi
...
if [ -f "skin.conf.patch" ]; then
    if ./config_patcher.py skin.conf.patch skin.conf; then
        log "✓ Configuration patched successfully"
    else
        log "✗ Configuration patching failed"
        exit 1
    fi
else
    log "⚠ No skin.conf.patch found - skipping configuration patching"
fi

# Restart WeeWX
log "Restarting WeeWX..."
if sudo systemctl restart weewx; then
    log "✓ WeeWX restarted successfully"
else
    log "✗ WeeWX restart failed"
    exit 1
fi
...
```bash
sudo chmod +x /usr/local/bin/update-neowx.sh
```
...
0 3 * * 0 /usr/local/bin/update-neowx.sh

# Run every Monday at 2:00 AM
0 2 * * 1 /usr/local/bin/update-neowx.sh

# Run first day of every month at 3:00 AM
0 3 1 * * /usr/local/bin/update-neowx.sh
```
...
```bash
sudo /usr/local/bin/update-neowx.sh
```

Check the log file:

```bash
sudo tail -f /var/log/neowx-update.log
```
...
| Bi-weekly | `0 3 * * 0/2` | Every other Sunday |
| Monthly | `0 3 1 * *` | First day of month at 3:00 AM |
| Daily | `0 3 * * *` | Every day at 3:00 AM |

### Update Notifications (Optional)

To receive email notifications when updates run, add email functionality to the script:

```bash
#!/bin/bash
# NeoWX Material automatic update script with email notifications
...
if [ $? -eq 0 ]; then
    send_notification "NeoWX Update Success" "NeoWX Material was updated successfully at $(date)"
    log "✓ Notification sent"
else
    send_notification "NeoWX Update Failed" "NeoWX Material update failed. Check logs: $LOG_FILE"
    log "✗ Update failed - notification sent"
fi
```

**Note:** Requires `mailutils` or `sendmail` installed:
```bash
...
```bash
# View entire log
sudo cat /var/log/neowx-update.log

# View last 20 lines
sudo tail -20 /var/log/neowx-update.log
...
sudo grep "update-neowx" /var/log/syslog  # Debian/Ubuntu
sudo grep "update-neowx" /var/log/cron    # RHEL/CentOS
```

### Troubleshooting Automated Updates

**Problem:** Script not running at scheduled time
...
3. Ensure script has execute permissions:
   ```bash
   ls -l /usr/local/bin/update-neowx.sh
   ```

**Problem:** Permission denied errors

**Solutions:**
1. Ensure script runs as root (use `sudo crontab -e`)
2. Check file ownership and permissions
3. Verify WeeWX user has necessary permissions

**Problem:** Updates run but settings not applied

**Solutions:**
1. Check if `skin.conf.patch` exists
2. Verify patch file syntax
3. Review log file for errors:
   ```bash
   sudo tail -50 /var/log/neowx-update.log
   ```

### Best Practices for Automated Updates

✅ **Test first** - Run the script manually before scheduling
✅ **Check logs** - Regularly review update logs
✅ **Off-peak hours** - Schedule during low-traffic times (e.g., 2-4 AM)
✅ **Weekly updates** - Balance between staying current and stability
✅ **Backup patch file** - Keep a backup of your `skin.conf.patch`
✅ **Monitor WeeWX** - Ensure station continues running after updates
✅ **Email notifications** - Get notified of update success/failure
